...
public class RDSJanitorResourceTracker implements JanitorResourceTracker {
	 ...
	 *
	 */
	public RDSJanitorResourceTracker(String dbDriver, String dbUser, String dbPass, String dbUrl, String dbTable) {
	...
	/**
	 * Instantiates a new RDS janitor resource tracker. This constructor is
	...
	public void addOrUpdate(Resource resource) {
		...
		LOGGER.debug(String.format("Saving resource %s to RDB table %s in region %s", resource.getId(), table,
				resource.getRegion()));
		String json;
		try {
		...
		if (orig == null) {
			...
			int updated = this.jdbcTemplate.update(sb.toString(), resource.getId(),
					...
					emailValue(resource.getOwnerEmail()), value(resource.getDescription()),
					value(resource.getState().toString()), value(resource.getTerminationReason()),
					value(resource.getExpectedTerminationTime()), value(resource.getActualTerminationTime()),
					value(resource.getNotificationTime()), value(resource.getLaunchTime()),
			...
			int updated = this.jdbcTemplate.update(sb.toString(), resource.getResourceType().toString(),
					...
					value(resource.getExpectedTerminationTime()), value(resource.getActualTerminationTime()),
					value(resource.getNotificationTime()), value(resource.getLaunchTime()),
					value(resource.getMarkTime()), value(resource.isOptOutOfJanitor()), json, resource.getId(),
					resource.getRegion());
	...
	/**
	 * Returns a list of AWSResource objects. You need to override this method
	 * if more specific resource types (e.g. subtypes of AWSResource) need to be
	...
	private Resource mapResource(ResultSet rs) throws SQLException {
		...
		try {
			...
			String expectedTerminationTime = millisToFormattedDate(
					rs.getString(AWSResource.FIELD_EXPECTED_TERMINATION_TIME));
			String actualTerminationTime = millisToFormattedDate(
					rs.getString(AWSResource.FIELD_ACTUAL_TERMINATION_TIME));
	...
	public Resource getResource(String resourceId) {
		...
		LOGGER.debug(String.format("Query is '%s'", query));
		List<Resource> resources = jdbcTemplate.query(query.toString(), new String[] { resourceId },
	...
	public Resource getResource(String resourceId, String region) {
		...
		LOGGER.debug(String.format("Query is '%s'", query));
		List<Resource> resources = jdbcTemplate.query(query.toString(), new String[] { resourceId, region },
				new RowMapper<Resource>() {
					public Resource mapRow(ResultSet rs, int rowNum) throws SQLException {
						return mapResource(rs);
					}
	...
	public void init() {
		try {
			...
			String sql = String.format(
					"create table if not exists %s (" + " %s varchar(255), " + " %s varchar(255), "
							+ " %s varchar(25), " + " %s varchar(255), " + " %s varchar(255), " + " %s varchar(25), "
							+ " %s varchar(255), " + " %s BIGINT, " + " %s BIGINT, " + " %s BIGINT, " + " %s BIGINT, "
							+ " %s BIGINT, " + " %s varchar(8), " + " %s varchar(4096) )",
					table, AWSResource.FIELD_RESOURCE_ID, AWSResource.FIELD_RESOURCE_TYPE, AWSResource.FIELD_REGION,
					AWSResource.FIELD_OWNER_EMAIL, AWSResource.FIELD_DESCRIPTION, AWSResource.FIELD_STATE,
					AWSResource.FIELD_TERMINATION_REASON, AWSResource.FIELD_EXPECTED_TERMINATION_TIME,
					AWSResource.FIELD_ACTUAL_TERMINATION_TIME, AWSResource.FIELD_NOTIFICATION_TIME,
					AWSResource.FIELD_LAUNCH_TIME, AWSResource.FIELD_MARK_TIME, AWSResource.FIELD_OPT_OUT_OF_JANITOR,
					"additionalFields");
